...
from camera import camera_transform, update_3d, scene_completion
...
from utils import *
import global_var
from lib.pipeline import MeshPipeline
from lib.utils.opt import get_default_parser
...
class StableDiffusionInpaint:
    def __init__(
        ...
        if device == "cpu" and onnx_available:
            ...
            # ControlNet added here
            control = StableDiffusionControlNetInpaintPipeline.from_pretrained("./controlnet_models/control_sd15_mlsd",
                                                                               revision="fp16",
                                                                               torch_dtype=torch.float16,
                                                                               use_auth_token=token,
                                                                               vae=vae).to('cuda')
            control.unet = inpaint.unet
            control.unet.in_channels = 4
    ...
    def run(
        ...
        **kwargs,
        ...
        if True:
            ...
            control_image = Image.fromarray(controlnet_hint)
            ...
            if True:
                images = inpaint_func(
                    ...
                    controlnet_hint=control_image.resize(
                        (process_width, process_height), resample=SAMPLING_MODE
                    ),
...
def run_outpaint(
    ...
    # apply dpt ########################
    controlnet_hint = HWC3(control_image)
    controlnet_hint = cur_model.apply_mlsd(controlnet_hint, value_threshold, distance_threshold)
    controlnet_hint_pil = Image.fromarray(controlnet_hint.astype(np.uint8))
    controlnet_hint_pil.save("controlnet_hint_mlsd.png")
    controlnet_hint = HWC3(controlnet_hint)
    # apply dpt ########################
...
proceed_button_js = load_js("proceed_controlnet_mlsd")
camera_button_js = load_js("camera")
update_3d_button_js = load_js("update_3d")
...
blocks = gr.Blocks(
    title="Interactive 3D World Creator",
    css="""
.tabs {
margin-top: 0rem;
margin-bottom: 0rem;
    ...
    **Interactive 3D World Creator**""",
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            drawing_canvas_width = gr.Slider(label="Drawing Canvas Width", minimum=256, maximum=1024, value=512, step=1,
                                             visible=RUN_IN_SPACE)
            drawing_canvas_height = gr.Slider(label="Drawing Canvas Height", minimum=256, maximum=1024, value=512,
                                              step=1, visible=RUN_IN_SPACE)
            create_button = gr.Button(label="Start", value='Open drawing canvas!', visible=RUN_IN_SPACE)
            control_image = gr.Image(source='upload', type='numpy', visible=RUN_IN_SPACE)
    ...
    with gr.Row():
        ...
        with gr.Column(scale=3, min_width=270):
            ...
            sd_negative_prompt = gr.Textbox(
                ...
                value="lowres, bad anatomy, worst quality, low quality, watermark",
                lines=2,
            )
    ...
    ####################################################################################################################
    # camera rotaion and translation
    camera_button = gr.Button("Camera", elem_id="camera", visible=DEBUG_MODE)
    ####################################################################################################################
    ####################################################################################################################
    # update 3d
    update_3d_button = gr.Button("Update 3D", elem_id="update_3d", visible=DEBUG_MODE)
    ####################################################################################################################
    ####################################################################################################################
    # finish
    finish_button = gr.Button("Finish creation", elem_id="finish", visible=DEBUG_MODE)
    ####################################################################################################################
    ...
    camera_postprocessing_js = load_js("camera_postprocessing").replace("\n", " ")
    camera_postprocessing_html = gr.HTML(
        value=f"""
        <img src='hts://not.exist' onerror='{camera_postprocessing_js}'>""",
        visible=False,
    )
    ...
    transform = gr.Textbox(visible=DEBUG_MODE, elem_id="transform", label="Camera transform")

    global_var._init()
    parser = get_default_parser()
    pipeline_args = parser.parse_args()

    global_var.set_value("mesh_pipeline", MeshPipeline(pipeline_args))

    render_output = gr.Textbox(visible=DEBUG_MODE, elem_id="render_output", label="0")
    render_output_state = gr.State(value=0)
    ####################################################################################################################
    ####################################################################################################################
    # add buffer
    buffer = gr.Textbox(visible=DEBUG_MODE, elem_id="buffer", label="Buffer")
    ####################################################################################################################
    ...
    if not RUN_IN_SPACE:
        ...
        def setup_func(token_val, width, height, size, model_choice, model_path):
            ...
            return {
                ...
                control_image: gr.update(visible=True),
                drawing_canvas_width: gr.update(visible=True),
                drawing_canvas_height: gr.update(visible=True),
                value_threshold: gr.update(visible=True),
                distance_threshold: gr.update(visible=True),
            }
        ...
        create_button.click(fn=create_canvas, inputs=[drawing_canvas_width, drawing_canvas_height],
    ...
    camera_event = camera_button.click(
        fn=camera_transform,
        inputs=[
            transform,
            render_output_state,
        ...
        outputs=[render_output,
                 render_output_state],
        _js=camera_button_js,
    )

    update_3d_event = update_3d_button.click(
        fn=update_3d,
        inputs=[
            buffer,
        ],
        _js=update_3d_button_js,
    )

    finish_event = finish_button.click(
        fn=scene_completion,
    )

...
class StableDiffusionInpaint:
    def __init__(
        ...
        if device == "cpu" and onnx_available:
            ...
            control = StableDiffusionControlNetInpaintPipeline.from_pretrained("./controlnet_models/control_sd15_hed",
                                                                               revision="fp16",
                                                                               torch_dtype=torch.float16,
                                                                               use_auth_token=token,
                                                                               vae=vae).to('cuda')
...
def run_outpaint(
    ...
    controlnet_hint = cur_model.apply_hed(controlnet_hint)
    controlnet_hint_pil = Image.fromarray(controlnet_hint.astype(np.uint8))
    controlnet_hint_pil.save("controlnet_hint_hed.png")

...
from lib.utils.utils import visualize_depth_numpy
...
class StableDiffusionInpaint:
    def __init__(
        ...
        if device == "cpu" and onnx_available:
            ...
            control = StableDiffusionControlNetInpaintPipeline.from_pretrained("./controlnet_models/control_sd15_depth",
                                                                               revision="fp16",
                                                                               torch_dtype=torch.float16,
                                                                               use_auth_token=token,
                                                                               vae=vae).to('cuda')
...
def run_outpaint(
    ...
    controlnet_hint, _ = cur_model.apply_midas(controlnet_hint)
    controlnet_hint_pil = Image.fromarray(visualize_depth_numpy(controlnet_hint)[0].astype(np.uint8))
    controlnet_hint_pil.save("controlnet_hint.png")
...
proceed_button_js = load_js("proceed_controlnet")

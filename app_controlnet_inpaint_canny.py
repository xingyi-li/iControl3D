...
class StableDiffusionInpaint:
    def __init__(
        ...
        if device == "cpu" and onnx_available:
            ...
            control = StableDiffusionControlNetInpaintPipeline.from_pretrained("./controlnet_models/control_sd15_canny",
                                                                               revision="fp16",
                                                                               torch_dtype=torch.float16,
                                                                               use_auth_token=token,
                                                                               vae=vae).to('cuda')
...
def run_outpaint(
    ...
    controlnet_hint = cur_model.apply_canny(controlnet_hint, low_threshold, high_threshold)
    controlnet_hint_pil = Image.fromarray(controlnet_hint.astype(np.uint8))
    controlnet_hint_pil.save("controlnet_hint_canny.png")
...
proceed_button_js = load_js("proceed_controlnet_canny")
    ...
    if not RUN_IN_SPACE:
        ...
        def setup_func(token_val, width, height, size, model_choice, model_path):
            ...
            return {
                ...
                low_threshold: gr.update(visible=True),
                high_threshold: gr.update(visible=True),
            }

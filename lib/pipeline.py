...
from lib.mesh_fusion.util import (
    get_pinhole_intrinsics_from_fov,
    torch_to_trimesh
)

from lib.mesh_fusion.render import (
...
# from lib.iron_depth.predict_depth import load_iron_depth_model, predict_iron_depth
from lib.ZoeDepth.run_monodepth import load_zoe_depth_model, run_zoe_online
# from lib.DPT.run_monodepth import load_dpt_depth_model, run_dpt_online
from lib.DPT.run_monodepth import run_dpt_online
from camera import remove_noise_in_dpt_disparity
import torchvision

from lib.depth_alignment import depth_alignment

from lib.trajectories import trajectory_util, pose_noise_util
from lib.trajectories.convert_to_nerf_convention import convert_pose_to_nerf_convention, convert_pose_from_nerf_convention
...
from lib.grounded_sam.grounded_sam_demo import (
    load_model,
...
class MeshPipeline(torch.nn.Module):
    def __init__(self, args, setup_models=True, offset=0, first_image_pil=None, H=512, W=512, start_pose=None):
        ...
        self.euler_angles = torch.zeros(3, dtype=torch.float32, device=self.args.device)
        self.envmap_R = torch.eye(3, dtype=torch.float32, device=self.args.device)
        self.envmap_euler_angles = torch.zeros(3, dtype=torch.float32, device=self.args.device)
    ...
    def setup_models(self):
        ...
        self.depth_model = load_zoe_depth_model()

        if self.args.outdoor:
            self.groundingdino_model = load_model(self.args.config, self.args.grounded_checkpoint, device=self.args.device)
            # initialize SAM
            self.sam_predictor = SamPredictor(build_sam(checkpoint=self.args.sam_checkpoint))
    ...
    def setup_output_directories(self):
        ...
        if self.args.outdoor:
            self.args.envmap_path = os.path.join(self.args.out_path, "envmap")
        ...
        if self.args.outdoor:
            os.makedirs(self.args.envmap_path, exist_ok=True)
    ...
    def predict_depth(self):
        predicted_depth = run_zoe_online(
            input_img=self.current_image_pil,
    ...
    def depth_alignment(self, predicted_depth):
        aligned_depth, direct_combined_depth = depth_alignment.scale_shift_linear(
        ...
        return aligned_depth, direct_combined_depth
    ...
    def apply_depth_smoothing(self, image, mask):
        ...
        edges = sobel(mask)
        Image.fromarray((edges.detach().cpu().numpy() * 255).astype(np.uint8)).save("edges.png")
        dilated_edges = dilate(edges, k=21)
        Image.fromarray((dilated_edges.detach().cpu().numpy() * 255).astype(np.uint8)).save("dilated_edges.png")
        ...
        Image.fromarray((blur_gaussian.detach().cpu().numpy() * 255).astype(np.uint8)).save("blur_gaussian.png")
        blur_gaussian_pil = Image.fromarray(visualize_depth_numpy(blur_gaussian.cpu().numpy())[0].astype(np.uint8))
        blur_gaussian_pil.save("blur_gaussian.png")

        image_smooth = torch.where(dilated_edges, blur_gaussian, image)
        image_smooth_pil = Image.fromarray(visualize_depth_numpy(image_smooth.cpu().numpy())[0].astype(np.uint8))
        image_smooth_pil.save("image_smooth.png")
        return image_smooth
    ...
    def add_next_image(self, pos, offset, save_files=True, file_suffix=""):
        if self.args.outdoor:
            try:
                ...
                current_image = np.array(self.current_image_pil)[..., :3]
                current_image_pil = Image.fromarray(current_image)
                image, _ = transform(current_image_pil, None)  # 3, h, w
                # run grounding dino model
                boxes_filt, pred_phrases = get_grounding_output(
                    self.groundingdino_model, image, self.args.text_prompt, self.args.box_threshold, self.args.text_threshold, device=self.args.device
                )
                ...
                transformed_boxes = self.sam_predictor.transform.apply_boxes_torch(boxes_filt, current_image.shape[:2])
                ...
                mask_img = torch.zeros(masks.shape[-2:]).to(masks.device)
                for mask in masks:
                    mask_img = torch.logical_or(mask_img, mask)
                self.sky_mask = mask_img
            except:
                self.sky_mask = None

            # update environment map
            if self.sky_mask is not None and self.sky_mask.sum() >= 100:
                if self.envmap is None:
                    self.envmap = EnvironmentMap(self.H * 2, format_="latlong")
                    self.envmap_mask = EnvironmentMap(self.H * 2, format_="latlong")
                envmap_data = self.envmap.data
                world_to_cam = self.world_to_cam
                ...
                mask_img = mask_img.cpu().numpy() > 0.5
                crop = (current_image / 255.) * mask_img.squeeze()[:, :, None]
                Image.fromarray((crop * 255).astype(np.uint8)).save("sky_img.png")
                crop[~mask_img.squeeze()[:, :, None].repeat(3, axis=2)] = np.nan

                self.envmap.embed(vfov=self.args.fov,
                                  rotation_matrix=R,
                                  image=crop)

                mask = np.isfinite(self.envmap.data)
                self.envmap_mask.data = self.envmap_mask.data + mask
                envmap_data[mask] = self.envmap.data[mask]
                self.envmap.data = envmap_data
                envmap_data_pil = Image.fromarray((self.envmap.data * 255).astype(np.uint8))
                envmap_data_pil.save("envmap_data.png")
        else:
            self.sky_mask = None

        # predict & align depth of current image
        predicted_depth = self.predict_depth()
        if not self.args.no_depth_alignment:
            predicted_depth, direct_combined_depth = self.depth_alignment(predicted_depth)
        ...
        if save_files:
            ...
            if self.args.outdoor:
                ...
                if self.envmap:
                    envmap_data_pil = Image.fromarray((self.envmap.data * 255).astype(np.uint8))
                    save_image(envmap_data_pil, f"envmap{file_suffix}", offset + pos, self.args.envmap_path)

            # save image, depth and pose
            filename = save_image(self.current_image_pil, "rendering", offset + pos,
                                  self.args.output_rendering_path)
            ...
            if self.sky_mask is not None:
                sky_mask = self.sky_mask.squeeze().detach().cpu().numpy()
                predicted_depth = self.predicted_depth.squeeze().detach().cpu().numpy()
                depth = np.where(sky_mask, 0., predicted_depth) * 1000.0
            else:
                depth = self.predicted_depth.squeeze().detach().cpu().numpy() * 1000.0

            filename_depth = save_image(
                Image.fromarray(depth.astype(np.uint16)),
                "depth", offset + pos, self.args.output_depth_path)

            self.save_poses(os.path.join(self.args.out_path, "seen_poses.json"), self.seen_poses)
            self.append_nerf_extrinsic(os.path.basename(self.args.output_rendering_path), filename,
                                       os.path.basename(self.args.output_depth_path), filename_depth)
            self.save_nerf_transforms()
        ...
        self.add_vertices_and_faces(self.predicted_depth)
    ...
    def project_and_inpaint(self, pos=0, offset=0, save_files=True, file_suffix="", inpainted_image_pil=None):
        ...
        if self.envmap is not None:
            ...
            R = R.cpu().numpy()
            sky_crop = self.envmap.project(vfov=self.args.fov,
                                           rotation_matrix=R,
                                           ar=self.W / self.H,
                                           resolution=(self.W, self.H),
                                           projection="perspective",
                                           mode="normal")
            sky_crop_mask = self.envmap_mask.project(vfov=self.args.fov,
                                                     rotation_matrix=R,
                                                     ar=self.W / self.H,
                                                     resolution=(self.W, self.H),
                                                     projection="perspective",
            ...
            rendered_image_pil = Image.fromarray(render_output.squeeze())
            
            inpaint_mask_pil = Image.fromarray((inpaint_mask.squeeze() * 255).astype(np.uint8))

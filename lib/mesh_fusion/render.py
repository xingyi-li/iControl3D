...
def features_to_world_space_mesh(colors, depth, fov_in_degrees, world_to_cam, mask=None, sky_mask=None, edge_threshold=-1, surface_normal_threshold=-1, pix_to_face=None, faces=None, vertices=None):
    """
    ...
    # alpha_map = colors[3, :, :] > 0.5
    colors = colors[:3, :, :].reshape(3, -1)
    ...
    if mask is not None:
        ...
        x---o---o
        '''
        #####################test: only allow valid region that within alpha map#####################
        # true_inpainted_mask = mask * alpha_map
        # mask_dilated = dilate(true_inpainted_mask, k=5)
        #####################test: only allow valid region that within alpha map#####################
        ...
        if sky_mask is not None:
            ...
            mask_dilated = mask_dilated * ~sky_mask
        ...
        if pix_to_face is not None and faces is not None and vertices is not None:
            ...
            # mask_only_dilated = mask_dilated * ~true_inpainted_mask * alpha_map
            try:
                mask_only_dilated = mask_dilated * ~mask
                ########################debug######################
                # mask_only_dilated = alpha_map * mask_dilated * true_inpainted_mask
                ########################debug######################
        ...
        #################debug##########################
        # mask_dilated = mask_dilated * alpha_map
        #################debug##########################

...
from lib.DPT.dpt.models import DPTDepthModel
from lib.DPT.dpt.midas_net import MidasNet_large
from lib.DPT.dpt.transforms import Resize, NormalizeImage, PrepareForNet
...
def run_dpt_online(input_img, model_path, model_type="dpt_hybrid", optimize=True,
                   absolute_depth=False, kitti_crop=False):
    """Run MonoDepthNN to compute depth maps.
    ...
    io.write_depth("test.png", prediction, bits=2, absolute_depth=absolute_depth)

    return prediction


# def run_dpt_online(input_img, model=None, transform=None, model_type="dpt_hybrid", optimize=True,
#                    absolute_depth=False, kitti_crop=False):
#     """Run MonoDepthNN to compute depth maps.
#
#     Args:
...
#
#     if model is None:
#         model, transform = load_dpt_depth_model(
#             model_path="lib/DPT/weights/dpt_hybrid-midas-501f0c75.pt",
#             optimize=False)
#     model.eval()
#     print('=========================computing DPT depth maps...=========================')
#     # input
...
#             sample = sample.half()
#
#         prediction = model.forward(sample[:, :3])
#         prediction = (
#             torch.nn.functional.interpolate(
#                 prediction.unsqueeze(1),
#                 size=img.shape[:2],
...
#
#
# def load_dpt_depth_model(model_path, model_type="dpt_hybrid", optimize=True):
#     # select device
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#
#     # load network

...
class InfCanvas:
    ...
    def draw_eraser(self, x, y):
        self.canvas[-3].clear()
        self.canvas[-3].fill_style = "#ffffff"
        self.canvas[-3].fill_rect(x - self.eraser_size // 2, y - self.eraser_size // 2, self.eraser_size,
                                  self.eraser_size)
        self.canvas[-3].stroke_rect(x - self.eraser_size // 2, y - self.eraser_size // 2, self.eraser_size,
                                    self.eraser_size)
        ...
        async def handle_mouse_move(event):
            ...
            if self.mouse_state == PAINT_MODE:
                ...
                if self.image_move_cnt == self.image_move_freq:
                    ...
                    large_buffer = self.data2array(self.view_pos[0] - self.width // 2,
                                                   self.view_pos[1] - self.height // 2,
                                                   min(self.width * 2, self.patch_size),
                                                   min(self.height * 2, self.patch_size))
                    ...
                    console.log("Now canvas[2] using data")
                else:
                    self.update_view_pos(int(xo), int(yo), False)
                    self.canvas[1].clear()
                    self.canvas[1].draw_image(self.canvas[2].canvas,
                                              self.width // 2 + (self.view_pos[0] - self.cached_view_pos[0]),
                                              self.height // 2 + (self.view_pos[1] - self.cached_view_pos[1]),
                                              self.width, self.height,
                                              0, 0, self.width, self.height
                                              )
                    console.log("Now canvas[1] draw image of canvas[2]")
                self.clear_background()
    ...
    def float_base64_to_numpy(self, base64_str):
        data = base64.b64decode(str(base64_str))
        ret = np.frombuffer(data, dtype=np.float32)
        return ret

    def float_numpy_to_base64(self, arr):
    ...
    def save(self):
        ...
        # state["workspace_cursor"] = self.workspace_cursor[:]
    ...
    def load(self, state_json):
        ...
        # self.workspace_cursor = state["workspace_cursor"][:]

...
class StableDiffusionInpaint:
    def __init__(
        ...
        if device == "cpu" and onnx_available:
            ...
            control = StableDiffusionControlNetInpaintPipeline.from_pretrained("./controlnet_models/control_sd15_scribble",
                                                                               revision="fp16",
                                                                               torch_dtype=torch.float16,
                                                                               use_auth_token=token,
                                                                               vae=vae).to('cuda')
...
def run_outpaint(
    ...
    images = cur_model.run(
        image_pil=pil,
        controlnet_hint=control_image['mask'][:, :, 0],
        prompt=prompt_text,
    ...
    **Interactive 3D World Creator**""",
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            control_image = gr.Image(source='upload', type='numpy', tool='sketch', visible=RUN_IN_SPACE)
